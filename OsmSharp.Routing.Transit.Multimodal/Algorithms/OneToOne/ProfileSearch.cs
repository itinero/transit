...
namespace OsmSharp.Routing.Transit.Multimodal.Algorithms.OneToOne
{
    ...
    public class ProfileSearch : RoutingAlgorithmBase, IConnectionList
    {
        ...
        /// </summary>
        public ProfileSearch(MultimodalDb db, DateTime departureTime,
        ...
        {
            var newStatus = new Profile()
            {
                PreviousConnectionId = Constants.NoConnectionId,
                Seconds = (int)seconds + (int)(_departureTime - _departureTime.Date).TotalSeconds
            };

            ProfileCollection existingStatus;
            if (_forwardProfiles.TryGetValue((int)stop, out existingStatus))
            { // a status already exists, compare.
                if (existingStatus.Seconds > newStatus.Seconds)
                {
                    _forwardProfiles[(int)stop] = new ProfileCollection(newStatus);
                }
            }
            else
            {
                _forwardProfiles.Add((int)stop, new ProfileCollection(newStatus));
            }
            return false;
        }
        ...
        private bool ReachedStopBackward(uint stop, float seconds)
        {
            ...
            if (_backwardProfiles.TryGetValue((int)stop, out existingStatus))
            {
                if (existingStatus.Seconds > newStatus.Seconds)
                {
                    _backwardProfiles[(int)stop] = newStatus;
                }
            }
            else
            {
                _backwardProfiles.Add((int)stop, newStatus);
            }
            return false;
        }
