...
namespace OsmSharp.Routing.Transit.Multimodal.Algorithms.Resolving
{
    ...
    {
        private readonly RouterDataSourceBase<Edge> _graph;
        ...
        /// </summary>
        public PathSegmentVisitListResolver(RouterDataSourceBase<Edge> graph, Vehicle vehicle)
        {
            _graph = graph;
            _vehicle = vehicle;
        }
        ...
        public RouterPoint Resolve(GeoCoordinate location)
        {
            GeoCoordinate closestLocation;
            var vertex = this.SearchClosest(location, out closestLocation);
        ...
        private uint SearchClosest(GeoCoordinate location, out GeoCoordinate closestLocation)
        {
            ...
            while (arcs.MoveNext())
            {
                    ...
                    _graph.GetVertex(arcs.Vertex2, out toLatitude, out toLongitude))
                {
                    ...
                    var distance = location.DistanceReal(vertexCoordinate).Value;
                    ...
                    distance = location.DistanceReal(vertexCoordinate).Value;
                    if (distance < bestDistance)
                    { // the distance found is closer.
                        bestDistance = distance;
